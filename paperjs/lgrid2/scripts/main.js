...
(function () {
    ...
    var config = {
        ...
        CANVAS_BACKGROUND_COLOR:"dimgray",

        CANVAS_WIDTH: 1280,
        CANVAS_HEIGHT: 1280, //16:9 aspect ratio
        SCALE_CANVAS: false,
        //TEMPLATE: "../_templates/mesh_head_color.png",
        MASK_TEMPLATE: "../_templates/masks/mesh.gif",
        TEMPLATE: "../_templates/grid_2.jpg",
        ANIMATE: false,
        TRACK_MOUSE:false,
    ...
    var getColor = function(point) {
        let m = maskData.getHex(point);

        if(m != "#000000") {
            return null;
        }

        return pixelData.getColor(point);

    }

    var renderCircle = function(r, cellWidth, cellHeight, padding) {

        var col = getColor(r.center);
    ...
    window.onload = function () {
        ...
        pdl.load(config.TEMPLATE,
            function(pd) {
                ...
                let pdl2 = new PixelDataLoader(config.CANVAS_WIDTH, config.CANVAS_HEIGHT);
                pdl2.load(config.MASK_TEMPLATE,
                    function(pd2) {
