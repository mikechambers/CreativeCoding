...
export default class Gradient {
	...
	//todo: need to be able to pass in option  to not cache pixel
	//data in case we want to display gradient and not access colors
	create() {
		...
		this._pixelData = new PixelData(
			this._ctx.getImageData(x, y, w, h),
			false
		);
...
export function gradientFromName(gradientName, bounds, type) {
	...
	if (!colors) {
		console.log(
			`Gradient.createGradientFromName : unknown name : {$gradientName}`
		);
		return undefined;
	}
